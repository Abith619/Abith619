...
class PayPalCustomController(http.Controller):
    ...
    def lead_checkout(self, **post):
        lead_id = post.get('lead_id')
        token = post.get('token')
        _logger.info(f"Lead Checkout called with lead_id={lead_id} and token={token}")
        if not lead_id or not token:
            _logger.error("Missing lead_id or token.")
            return request.redirect('/shop')
        lead = request.env['crm.lead'].sudo().browse(int(lead_id))
        if not lead or not lead.exists():
            _logger.error("Lead not found.")
            return request.redirect('/shop')
        if lead.unique_token != token:
            _logger.error("Invalid or expired token.")
            return request.redirect('/shop')
        if not lead.course_id or not lead.course_id.product_id:
            _logger.error("Course or Product missing for lead.")
            return request.redirect('/shop')
