...
class dicom_view(models.Model):
    ...
    def run_dicom_viewer(self):
        filename = get_testdata_file("CT_small.dcm")
        ds = pydicom.dcmread(filename, force=True)
        data = plt.imshow(ds.pixel_array, cmap=plt.cm.bone)
        raise ValidationError(type(data))

    def open_dicom_viewer(self):
        url = 'http://localhost:3000/local'
        headers = {"Content-Type": "application/dicom"}
        file_path = self.upload_dcm.datas
        ...
        #     'name': "dicom.viewer",
        #     'view_mode': 'form',
        #     'res_model': 'dicom.viewer.wizard',
        #     'view_type': 'form',
        #     'type': 'ir.actions.act_window',
        #     'target':'new',
        #     'context':{
        #         'default_dicom_viewer_id' : self.id,
        #         }
        #     }

class DicomWizard(models.TransientModel):
    _name = 'dicom.viewer.wizard'

    dicom_viewer_id = fields.Many2one('dicom.view',string='Dicom Viewer')
    ...
    def open_dicom_viewer(self):
        self.ensure_one()
        return self.dicom_viewer_id.open_dicom_viewer

class DicomViewerButton(models.Model):
    _inherit='ir.actions.act_window'

    dicom_viewer = lambda self:self.env['dicom.view'].create({'report_name':'Dicom Viewer'})
    wizard = lambda self:self.env['dicom.viewer.wizard'].create({'dicom_viewer_id':dicom_viewer.id})
