...
export class MrpDisplayRecord extends Component {
    ...
    async validate() {
        const { resModel, resId } = this.props.record;
        let shouldAddToValidationStack = true;
        if (resModel === "mrp.workorder") {
        ...
        if (resModel === "mrp.production") {
            ...
            if (action && typeof action === "object") {
                action.context.skip_redirection = true;
                return this._doAction(action);
            } else if (action && typeof action === "boolean") {
                // If the action is a boolean, it means that the MO is already done by pre_mark_done
                // so it is not needed to add to validation stack despite it do button_mark_done twice.
                shouldAddToValidationStack = false;
                this.state.validated = true;
                this.env.reload();
            }
        }

        // Only add to validation stack if the record is not already validated 
        if (shouldAddToValidationStack){
