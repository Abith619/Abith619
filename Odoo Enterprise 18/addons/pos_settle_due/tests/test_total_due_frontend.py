...
class TestPointOfSaleFlow(TestPointOfSaleHttpCommon):
    ...
    def test_missing_pay_later_payment_method(self):
        self.env['res.partner'].create({'name': 'APartner'})
        self.main_pos_config.write({'payment_method_ids': [
            Command.set(self.main_pos_config.payment_method_ids.filtered(lambda pm: pm.type != 'pay_later').ids)
        ]})
        self.main_pos_config.with_user(self.pos_admin).open_ui()
        self.start_tour("/pos/ui?config_id=%d" % self.main_pos_config.id, 'test_missing_pay_later_payment_method', login='pos_admin')
